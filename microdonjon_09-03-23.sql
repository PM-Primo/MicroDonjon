...
-- Listage des données de la table microdonjon.chapitre : ~74 rows (environ)
...
-- Listage des données de la table microdonjon.chap_combat : ~9 rows (environ)
...
-- Listage des données de la table microdonjon.sortie_condition : ~32 rows (environ)
...
-- Listage des données de la table microdonjon.sortie_standard : ~109 rows (environ)
...
INSERT INTO `user` (`id`, `email`, `roles`, `password`, `pseudo`, `pvmax`, `pvactuels`, `gold`, `attaque`, `chapitre_en_cours`, `date_victoire`, `is_verified`) VALUES
	(16, 'toto@toto.web', '["ROLE_ADMIN"]', '$2y$13$tMnmNHNVBeHUPYQxsorioOk1oBJnmFL/AiY4S416ieaEcdYq7IuSi', 'toto', 100, 100, 75, 12, 64, NULL, 1);

-- Listage des données de la table microdonjon.user_chapitre : ~20 rows (environ)
...
-- Listage des données de la table microdonjon.user_item : ~2 rows (environ)
INSERT INTO `user_item` (`user_id`, `item_id`) VALUES
	(16, 1),
	(16, 2);

-- Listage des données de la table microdonjon.user_zone : ~11 rows (environ)
